...
function streamHls(req, res, next) {
	let streamPath = req.body.streamPath;
	console.log('[Streaming HLS started for path]:', streamPath);
  
	if (streamPath) {
	  this.nodeEvent.emit('streamHls', streamPath);
	  res.sendStatus(200);
	} else {
	  res.sendStatus(400);
	}
...
function postStreamTrans(req, res, next) {
  let streamPath = req.body.streamPath;
  console.log('post stream trans');

  if (streamPath) {
    this.nodeEvent.emit('onDynamicTrans', streamPath);
    res.sendStatus(200);
  } else {
    res.sendStatus(400);
  ...
  //   config.mp4
  // ) {
  //   const transServer = new NodeTransServer(config);
  //   transServer.run();
