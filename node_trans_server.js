...
class NodeTransServer {
  ...
  async run() {
    ...
    context.nodeEvent.on('onDynamicTrans', this.onDynamicTrans.bind(this));
    context.nodeEvent.on('streamHls', this.streamHls.bind(this));
    Logger.log(`Node Media Trans Server started for apps: [ ${apps}] , MediaRoot: ${this.config.http.mediaroot}, ffmpeg version: ${version}`);
  }

  streamHls(streamPath) {
